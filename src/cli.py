...
def get_conversion_options_interactive(video_info) -> ConversionOptions:
    ...
    if choice == 'Q':
        ...
        while True:
            speed_input = input(f"Enter speed (1.0-4.0) or press Enter for {suggested_speed:.2f}x: ").strip()
...
Examples:
  ...
  python cli.py --file video.mp4 --start 30 --duration 45
    ...
    parser.add_argument(
        '--file', '-f',
        type=str,
        help='Specific video file to convert'
    )
    ...
    parser.add_argument(
        '--start',
        type=float,
        help='Start time in seconds (for segment mode)'
    )
