...
def apply_vertical_layout(
    ...
    result = create_blur_background(
        ...
        blur_kernel, blur_sigma, brightness, darken
    )
...
def convert_to_shorts(
    ...
    try:
        ...
        logger.info(f"Video: {video_info.duration:.1f}s, {video_info.width}x{video_info.height}")
        ...
        if not video_info.is_short:
            if options.duration is not None:
                ...
                logger.info(f"Using segment: {options.start_time:.1f}s - {end_time:.1f}s")
...
def auto_convert(video_path: Path, output_path: Path) -> Path:
    ...
    if video_info.is_short:
        ...
        logger.info(f"Video is {video_info.duration:.1f}s, using {required_speed:.2f}x speed")
